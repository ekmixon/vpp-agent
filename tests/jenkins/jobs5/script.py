...
running_at_arm64_node_I = (
    f'{running_at_arm64_node_I}              ' + 'stage \'test\'\n')
running_at_arm64_node_II=''
running_at_arm64_node_II = (
    f'{running_at_arm64_node_II}              ' + 'stage \'test\'\n')
running_at_arm64_node_I_IPv4=''
running_at_arm64_node_I_IPv4 = (
    f'{running_at_arm64_node_I_IPv4}              ' + 'stage \'test\'\n')
running_at_arm64_node_II_IPv6=''
running_at_arm64_node_II_IPv6 = (
    f'{running_at_arm64_node_II_IPv6}              ' + 'stage \'test\'\n')
running_at_arm64_node_II_OTHER=''
running_at_arm64_node_II_OTHER = (
    f'{running_at_arm64_node_II_OTHER}              ' + 'stage \'test\'\n')
running_at_arm64_node_I_SFCIPv4=''
running_at_arm64_node_I_SFCIPv4 = (
    f'{running_at_arm64_node_I_SFCIPv4}              ' + 'stage \'test\'\n')
running_at_arm64_node_II_SFCIPv6=''
running_at_arm64_node_II_SFCIPv6 = (
    f'{running_at_arm64_node_II_SFCIPv6}              ' + 'stage \'test\'\n')

jenkins_project = '' + '- project:\n'
jenkins_project += '    name: ligato/vpp-agent all tests on arm64\n'
jenkins_project += '    jobs:\n'
with open("list_of_all_robot_tests") as f:
  for idx, i in enumerate(f):
    ...
    with open(i[:-1]) as myfile:
      is_expected_failure = 'ExpectedFailure' in myfile.read()
    ...
    jenkins_project = (
        f'{jenkins_project}          HOWTOBUILD_INCLTAGPRESENT: ' +
        ('--include ExpectedFailure' if is_expected_failure else '\' \'') +
        '\n')
    jenkins_project = (
        f'{jenkins_project}          HOWTOBUILD_EXCLTAGPRESENT: ' +
        ('--exclude ExpectedFailure' if is_expected_failure else '\' \'') +
        '\n')
    jenkins_project = (
        f'{jenkins_project}          inode_of_folder: ' + format(
            os.path.basename(os.path.normpath(odloz2[0])).upper(),
            "_<11s") + '\n')
    jenkins_project = (
        f'{jenkins_project}          date_of_jjb_generation: ' +
        datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S") + '\n')
    jenkins_project = (
        f'{jenkins_project}          name_of_test: {os.path.basename(i[:-7])}'
        + '\n')
    jenkins_project = f'{jenkins_project}          path_to_test: {i[:-1]}' + '\n'
    jenkins_project = (f'{jenkins_project}          local_variables_file: ' + (
        'arm64_local' if idx < rozdel_na_pozicii else 'arm64_II_local') + '\n')
    # following row is enough ti control the ARM64 node where test is going to be executed
    jenkins_project = (f'{jenkins_project}          arm64_node: ' +
                       ('147.75.72.194' if idx < rozdel_na_pozicii else
                        '147.75.98.202') + '\n')
    #I will divide the test to pipelines according IPv4/IPv6
    groupoftests=os.path.basename(os.path.normpath(odloz2[0])).upper()
    if groupoftests in ['CRUD', 'TRAFFIC']:
    ...
    elif groupoftests in ['CRUDIPV6', 'TRAFFICIPV6']:
